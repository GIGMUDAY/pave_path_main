...
export const CTA = () => {
  ...
  const handleCallSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessModal({
        title: "Success!",
        description: "Request submitted. We'll reach you within 24 hours. Thank you!",
      });
  ...
  const handleRedlineSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessModal({
        title: "Success!",
        description: "Redline task received. We'll reach you within 24 hours. Thank you!",
      });
          ...
          >
            ...
            <div className="flex rounded-xl bg-muted/50 dark:bg-card/50 border border-border/50 dark:border-border/50 p-1 mb-6 sm:mb-8">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2.5 sm:py-3 px-3 sm:px-4 rounded-lg font-medium transition-all touch-manipulation min-h-[44px] text-sm sm:text-base ${activeTab === 'call'
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2.5 sm:py-3 px-3 sm:px-4 rounded-lg font-medium transition-all touch-manipulation min-h-[44px] text-sm sm:text-base ${activeTab === 'redline'
                    ? 'bg-white dark:bg-card text-primary dark:text-foreground shadow-lg border border-border/50 dark:border-border/50'
                    : 'text-foreground/70 dark:text-foreground/70 hover:text-foreground dark:hover:text-foreground'
                  }`}
              >
      ...
      {successModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 bg-black/50 backdrop-blur-sm">
          <div
            role="alertdialog"
            aria-modal="true"
            className="relative w-full max-w-md rounded-2xl bg-card border border-border/70 shadow-2xl p-6 sm:p-7 text-center"
          >
            <button
              aria-label="Close"
              onClick={() => setSuccessModal(null)}
              className="absolute right-3 top-3 rounded-full p-2 text-muted-foreground hover:text-foreground hover:bg-accent transition"
            >
              ×
            </button>
            <div className="flex flex-col items-center gap-3 sm:gap-4">
              ...
              <button
                onClick={() => setSuccessModal(null)}
                className="mt-2 w-full sm:w-auto px-5 py-3 rounded-lg bg-primary text-primary-foreground font-semibold hover:bg-primary/90 transition"
              >
                Close
              </button>
            </div>
