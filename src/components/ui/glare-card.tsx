...
export const GlareCard: React.FC<GlareCardProps> = ({ children, className }) => {
  ...
  return (
    <div
      ref={refElement}
      className={cn(
        "container relative isolate w-full transition-transform will-change-transform [contain:layout_style] [perspective:600px]",
        className
      )}
    ...
    >
      <div
        className="grid h-full origin-center overflow-hidden rounded-lg border border-border transition-transform will-change-transform hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.6]"
        style={{
      ...
      >
        ...
        <div
          className="transition-background will-change-background grid size-full opacity-[var(--opacity)] mix-blend-soft-light transition-opacity [clip-path:inset(0_0_1px_0_round_var(--radius))] [grid-area:1/1]"
          style={{
